function [Tree, childToParent, index] = TreePolicy_TSP(Tree, t_final, d_thr, e_thr, alpha, index, childToParent, costMat, type)
...
%% Expands from pre-decision to post-decision
    function ExpandAction()
        numNodes = numNodes + 1;
        Tree(numNodes) = State_TSP();
        [Omega, Tree(numNodes)] = generateObservations_TSP(Tree(index), Tree(numNodes), costMat, x, type);
        Tree(numNodes) = Tree(numNodes).PostDecisionState(Omega, numNodes);
        Tree(numNodes) = TransitionPre2Post_TSP(Tree(index), Tree(numNodes));
...
%% Expands from post-decision to pre-decision
    function ExpandObservation()
        numNodes = numNodes + 1;
        Tree(numNodes) = State_TSP();
        Tree(numNodes) = TransitionPost2Pre_TSP(Tree(index), Tree(numNodes), w);    % Realizes profit
        Tree(numNodes).realization = w;
        decisions = generateDecisions_TSP(Tree(numNodes), length(costMat));
