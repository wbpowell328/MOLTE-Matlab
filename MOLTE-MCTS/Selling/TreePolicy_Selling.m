function [Tree, childToParent, index] = TreePolicy_Selling(Tree, t_final, d_thr, e_thr, alpha, index, childToParent, forecast, type)
...
%% Expands from pre-decision to post-decision
    function ExpandAction()
        numNodes = numNodes + 1;
        Tree(numNodes) = State_Selling();
        Tree(numNodes) = TransitionPre2Post_Selling(Tree(index), Tree(numNodes), x);
        [Tree(numNodes), Omega] = generateObservations_Selling(Tree(numNodes), forecast.price(Tree(index).t), type, 0.5);
...
%% Expands from post-decision to pre-decision
    function ExpandObservation()
        ...
        numNodes = numNodes + 1;
        Tree(numNodes) = State_Selling();
        decisions = generateDecisions_Selling(0:6:min(Tree(index).inv, 6), Tree(index).inv, t_final - Tree(index).t);
        Tree(numNodes) = Tree(numNodes).PreDecisionState(decisions, numNodes);
        Tree(numNodes) = TransitionPost2Pre_Selling(Tree(index), Tree(numNodes), w);    % Realizes profit
