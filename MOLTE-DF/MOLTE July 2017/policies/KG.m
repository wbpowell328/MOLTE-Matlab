...
function [ mu_est, count, recommendedArm ] = KG( mu_0,beta_W,covM,samples, alpha, tune)

[M,N]=size(samples);
beta_est=zeros(M,1);
sigmatilde=zeros(M,1); 
...
beta_est=1./diag(covM,0);
 ...
 for i=1:N
     ...
     choice(i)=x;
     reward_sum=reward_sum+W;
     ereward_sum=ereward_sum+mu(x);
 end
