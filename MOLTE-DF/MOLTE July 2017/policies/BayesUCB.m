...
function [ mu_est, count, recommendedArm ] = BayesUCB( mu_0,beta_W,covM,samples, alpha, tune)
%N>>M
[M,N]=size(samples);
...
for x=1:M  % pull each arm once at the beginning
    count(x)=count(x)+1;
    W=samples(x, count(x));
...
for i=M+1:N
    ...
    quantile=1-1/(i*(log(N)^alpha));
    
    for j=1:M  % calculate the quantile for each arm 
      q(j)=norminv(quantile, mu_est(j), sigma(j));
    end
