    ...
    parfor i=1:numP 
     %for i=1:numP       
        ...
        for j=1:numTruth 
               ...
               [mu,beta_W]=truthGen(problemClass,noiseRatio, mu_0, covM, beta_W);     
               mu_ALL(j,:)=mu;
               [samples ] = sampleGen( mu, beta_W, N );
               [ mu_est(j,:,:), choices(:,:,j), ereward_sum(j,:), recommendedArm(j,:) ] = policyRun(mu_0,beta_W,covM, samples, policies, mu, tuned, alpha);
